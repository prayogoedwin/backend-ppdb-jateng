...
//untuk yang sudah verif
export const getDataPendaftarByWhereHanyaUntukCekLagi = async (req, res) => {
    ...
    const redis_key = `DataPendaftarCekLagi:IdSekolah:${sekolah_tujuan_id}--page:${page}--limit:${limit}--offset:${offset}`;
    try {
        ...
        const role = dataAdminNya.role_

        // const cacheNya = await redisGet(redis_key);
        const cacheNya = false;
        if (cacheNya && !nisn && role != 95) {
...
export const verifikasiPendaftar = async (req, res) => {
        const { id, is_verified, keterangan_verifikator, cek_list_dok, is_disabilitas, is_with_surat_sehat, status_shdk, is_usia_domisili, is_nama_ortu_sesuai, is_boleh_prestasi_khusus, alasan_batal_verifikasi, kabkota_id_mutasi} = req.body;
...
export const updatePendaftarKhususPrestasi = async (req, res) => {
    const { 
